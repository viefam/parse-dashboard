...
program.option(
  ...
  "the GraphQL server url of the app you would like to manage."
);
program.option(
  "--dev",
  "Enable development mode. This will disable authentication and allow non HTTPS connections. DO NOT ENABLE IN PRODUCTION SERVERS"
);
program.option(
  "--appName [appName]",
...
program.option(
  ...
  "set this flag when you are behind a front-facing proxy, such as when hosting on Heroku.  Uses X-Forwarded-* headers to determine the client's connection and IP address."
);
program.option(
  "--cookieSessionSecret [cookieSessionSecret]",
  "set the cookie session secret, defaults to a random string. You should set that value if you want sessions to work across multiple server, or across restarts"
);

program.parse(process.argv);

const host = program.host || process.env.HOST || "0.0.0.0";
const port = program.port || process.env.PORT || 4041;
...
const cookieSessionSecret =
  program.cookieSessionSecret ||
  process.env.PARSE_DASHBOARD_COOKIE_SESSION_SECRET;
const trustProxy =
  program.trustProxy || process.env.PARSE_DASHBOARD_TRUST_PROXY;
...
let configServerURL =
  program.serverURL || process.env.PARSE_DASHBOARD_SERVER_URL;
let configGraphQLServerURL =
  program.graphQLServerURL || process.env.PARSE_DASHBOARD_GRAPHQL_SERVER_URL;
let configMasterKey =
  program.masterKey || process.env.PARSE_DASHBOARD_MASTER_KEY;
...
let configUserPassword =
  program.userPassword || process.env.PARSE_DASHBOARD_USER_PASSWORD;
...
p.then(
  config => {
    ...
    if (config.data.iconsFolder && configFilePath) {
      config.data.iconsFolder = path.join(
        configFilePath,
        config.data.iconsFolder
      );
    }
    ...
    if (!configSSLKey || !configSSLCert) {
      ...
      server = app.listen(port, host, function() {
        console.log(
          `The dashboard is now available at http://${
            server.address().address
          }:${server.address().port}${mountPath}`
        );
      });
    } else {
      ...
      server = require("https")
        ...
        .listen(port, host, function() {
          console.log(
            `The dashboard is now available at https://${
              server.address().address
            }:${server.address().port}${mountPath}`
          );
        });
